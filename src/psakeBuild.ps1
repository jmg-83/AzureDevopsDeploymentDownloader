properties {
  $script = "$PSScriptRoot\AzureDevopsDeploymentDownloader.psm1"
}
...
task Deploy -depends Analyze, Test {
  Invoke-PSDeploy -Path '.\AzureDevopsDeploymentDownloader.psdeploy.ps1' -Force -Verbose:$VerbosePreference
}
